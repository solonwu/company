...
public final class FatcaXmlBuilder {
    ...
    /** 被動非金融外國實體（Passive NFFE，AcctHolderType 通常是 FATCA104）帳戶下的實質受益人。
    ...
    private CorrectableAccountReportType buildAccountReport(AccountReportSpec spec) {
        ...
        for (SubstantialOwnerSpec so : spec.getSubstantialOwners()) {
            ...
            if (so.getIndividual() != null) {
                owner.setIndividual(buildPersonParty(so.getIndividual()));
            } else if (so.getOrganisation() != null) {
                owner.setOrganisation(buildOrganisationParty(so.getOrganisation()));
            }
