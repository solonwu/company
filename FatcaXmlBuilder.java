...
import oecd.ties.fatca.v2.CorrectablePoolReportType;
...
import oecd.ties.fatca.v2.FatcaAcctPoolReportTypeEnumType;
...
import oecd.ties.fatca.v2.SubstantialOwnerType;
...
public final class FatcaXmlBuilder {
    ...
    /** 被動非金融外國實體（Passive NFFE，AcctHolderType 通常是 FATCA102）帳戶下的實質受益人。
     *  IRS schema 的 SubstantialOwner_Type 是 Individual／Organisation 二擇一（同 AccountHolder）。 */
    @Data
    @Builder
    public static class SubstantialOwnerSpec {
    ...
    public static class AccountReportSpec {
        ...
        private final List<SubstantialOwnerSpec> substantialOwners;
        /** 資料庫既有的 DocRefId（XML_CorrectableAccountReport_Type.DocRefID）；比照原 STP_toXML
         *  邏輯沿用此值（前綴 GIIN），未提供時才 fallback 產生新的隨機 UUID。 */
        ...
        private AccountReportSpec(AccountReportSpecBuilder b) {
            ...
            this.substantialOwners  = List.copyOf(b.substantialOwners);
            this.docRefId           = b.docRefId;
        }
        ...
        public static final class AccountReportSpecBuilder {
            ...
            private final List<SubstantialOwnerSpec> substantialOwners = new ArrayList<>();
            ...
            public AccountReportSpecBuilder docRefId(String v) { this.docRefId = v; return this; }
            ...
            public AccountReportSpecBuilder addSubstantialOwnerIndividual(IndividualSpec individual) {
                this.substantialOwners.add(SubstantialOwnerSpec.builder().individual(individual).build());
                return this;
            }
            public AccountReportSpecBuilder addSubstantialOwnerOrganisation(OrganisationSpec organisation) {
                this.substantialOwners.add(SubstantialOwnerSpec.builder().organisation(organisation).build());
                return this;
            }
    ...
    /** 具相同特徵（如美國指標之不合作帳戶、休眠帳戶）帳戶持有人的彙總（pool）申報，
     *  對應 IRS schema 的 CorrectablePoolReport_Type — 不逐戶列出個別帳戶明細，
     *  只申報彙總的帳戶數與帳戶餘額。 */
    @Data
    @Builder
    public static class PoolReportSpec {
        private int accountCount;
        private PoolReportCode poolReportType;
        private Money poolBalance;
        /** 資料庫既有的 DocRefId（XML_PoolReport.DocRefID）；未提供時 fallback 產生新的隨機 UUID。 */
        private String docRefId;
    }
    ...
    private final List<PoolReportSpec> poolReports = new ArrayList<>();
    ...
    /** @param year 申報年度（送出申報的年度）；ReportingPeriod 會是 year-1 的 12/31。 */
    ...
    public FatcaXmlBuilder addPoolReport(PoolReportSpec spec) {
        this.poolReports.add(spec);
        return this;
    }
    ...
    private MessageSpecType buildMessageSpec() {
        ...
        // reportYear 是「申報年度」（送出申報的年度），FATCA 一律申報前一年度的帳戶資料，
        // 故 ReportingPeriod（申報所屬年度的年底）要用 reportYear - 1 的 12/31。
        spec.setReportingPeriod(toXmlDate(LocalDate.of(reportYear - 1, 12, 31)));
    ...
    private FatcaType buildFatcaBody() {
        ...
        if (nilReport) {
            ...
            poolReports.forEach(spec -> group.getPoolReport().add(buildPoolReport(spec)));
        }
    ...
    private CorrectableAccountReportType buildAccountReport(AccountReportSpec spec) {
        ...
        report.setDocSpec(newDocSpec(DocTypeIndic.FATCA1, spec.getDocRefId()));
        ...
        for (SubstantialOwnerSpec so : spec.getSubstantialOwners()) {
            SubstantialOwnerType owner = new SubstantialOwnerType();
            if (so.getIndividual() != null) {
                owner.setIndividual(buildPersonParty(so.getIndividual(), spec.getAccountNumber()));
            } else if (so.getOrganisation() != null) {
                owner.setOrganisation(buildOrganisationParty(so.getOrganisation(), spec.getAccountNumber()));
            }
            report.getSubstantialOwner().add(owner);
        }
    ...
    private CorrectablePoolReportType buildPoolReport(PoolReportSpec spec) {
        if (spec.getPoolReportType() == null) {
            throw new IllegalStateException(
                    "FatcaXmlBuilder: pool report requires 'poolReportType' (IRS FatcaAcctPoolReportType_EnumType, "
                            + "e.g. FATCA201=Recalcitrant with U.S. Indicia) — refusing to emit a report the IRS "
                            + "schema would reject");
        }
        CorrectablePoolReportType report = new CorrectablePoolReportType();
        report.setDocSpec(newDocSpec(DocTypeIndic.FATCA1, spec.getDocRefId()));
        report.setAccountCount(BigInteger.valueOf(spec.getAccountCount()));
        report.setAccountPoolReportType(FatcaAcctPoolReportTypeEnumType.fromValue(spec.getPoolReportType().getXmlCode()));
        report.setPoolBalance(toMonetaryAmount(spec.getPoolBalance()));
        return report;
    }
    ...
    private AccountHolderType buildHolder(AccountReportSpec spec) {
        ...
        if (spec.getIndividual() != null) {
            holder.setIndividual(buildPersonParty(spec.getIndividual(), spec.getAccountNumber()));
            return holder;
        }

        OrganisationSpec s = spec.getOrganisation();
        holder.setOrganisation(buildOrganisationParty(s, spec.getAccountNumber()));
        if (!hasText(s.getAcctHolderType())) {
            throw new IllegalStateException(
                    "FatcaXmlBuilder: account '" + spec.getAccountNumber() + "' organisation holder requires "
                            + "'acctHolderType' (IRS FatcaAcctHolderType_EnumType, e.g. FATCA102=Passive NFFE, "
                            + "FATCA103=NPFFI) — refusing to emit a report the IRS schema would reject");
        }
    ...
    /**
     * IRS schema 的 PersonParty_Type / OrganisationParty_Type 都要求至少一個 Address 元素
     * （minOccurs 未標即預設 1，跟真正選填的 ResCountryCode／TIN 不同）——缺地址國別時在這裡
     * 直接拒絕，而不是悄悄不產生 &lt;Address&gt;，讓後續 XSD 驗證失敗時只有一句籠統的
     * "element Address is missing"、看不出是哪個帳號缺資料。
     */
    private PersonPartyType buildPersonParty(IndividualSpec s, String accountNumber) {
        ...
        if (!hasText(s.getAddressCountry())) {
            throw new IllegalStateException(
                    "FatcaXmlBuilder: account '" + accountNumber + "' individual holder requires "
                            + "'addressCountry' (IRS PersonParty_Type.Address is mandatory) — refusing to emit a "
                            + "report the IRS schema would reject");
        }
    ...
    private OrganisationPartyType buildOrganisationParty(OrganisationSpec s, String accountNumber) {
        ...
        if (!hasText(s.getAddressCountry())) {
            throw new IllegalStateException(
                    "FatcaXmlBuilder: account '" + accountNumber + "' organisation holder requires "
                            + "'addressCountry' (IRS OrganisationParty_Type.Address is mandatory) — refusing to "
                            + "emit a report the IRS schema would reject");
        }
        org.getAddress().add(buildAddress(s.getAddressCountry(), s.getAddressFree()));
        return org;
    }

    /** ReportingFI／Sponsor／NilReport 沒有對應的資料庫 DocRefId 可沿用（比照原 STP_toXML 邏輯，
     *  這幾個節點本來就是每次現生成一個新的隨機 UUID），故用此覆載。 */
    private DocSpecType newDocSpec(DocTypeIndic type) {
        return newDocSpec(type, null);
    }

    /**
     * 比照原 STP_toXML 預儲程序邏輯：DocRefId 一律是 {@code GIIN + "." + docRefId}。
     * {@code docRefId} 若有值（帳戶明細沿用資料庫 XML_CorrectableAccountReport_Type.DocRefID），
     * 直接接上 GIIN 前綴；若無值（ReportingFI／Sponsor／NilReport 沒有對應資料庫值可沿用），
     * 現生成一個新的隨機 UUID 再接上 GIIN 前綴。
     */
    private DocSpecType newDocSpec(DocTypeIndic type, String docRefId) {
        DocSpecType docSpec = new DocSpecType();
        docSpec.setDocTypeIndic(FatcaDocTypeIndicEnumType.fromValue(type.getCode()));
        String suffix = hasText(docRefId) ? docRefId : UUID.randomUUID().toString();
        docSpec.setDocRefId(giin.value() + "." + suffix);
        return docSpec;
    }

    /**
     * 資料庫的地址欄位有時含 CR/LF（例如分行輸入的多行地址），序列化成 XML 後會變成
     * {@code &#13;}/{@code &#10;} 字元參照——技術上合法，但為避免申報檔內嵌控制字元，
     * 送出前一律換成空白。
     */
    private static String normalizeAddressFree(String addressFree) {
        return addressFree.replaceAll("[\r\n]+", " ").trim();
    }

    private TINType buildTin(String tin, String issuedBy) {
        TINType t = new TINType();
        // 資料庫裡的 TIN 有時會含連字號（如 SSN 格式 123-45-6789），IRS FATCA XML 要求純數字/英數字，
        // 送出前一律去掉「-」。
        t.setValue(tin != null ? tin.replace("-", "") : null);
    ...
    private AddressType buildAddress(String countryCode, String addressFree) {
        ...
        if (addressFree != null) {
            addr.getContent().add(SFA_OBJECT_FACTORY.createAddressTypeAddressFree(normalizeAddressFree(addressFree)));
        }
        return addr;
    }
