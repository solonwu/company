 ...
 *
 * <p>目前僅支援銀行部（{@link FiType#BANK} / {@link FiType#BRANCH}）：XML_* 表沒有欄位可
 * 辨識帳戶屬於銀行部或信託部（與帳戶查詢畫面 {@code LegacyAccountQueryService} 的已知限制
 * 相同）。信託部邏輯待後續開發，呼叫 {@link FiType#TRUST} 會直接拋出例外，不會靜默回傳銀行部
 * 資料（避免把帳戶歸錯部門這種申報正確性風險）。
 */
@Service
...
public class SptXmlService {
     ...
     * @param fiType              機構類型；僅支援 BANK / BRANCH
     * @return FATCA XML 字串
     * @throws IllegalStateException fiType 為 TRUST（尚未實作）
    ...
    public String generateXml(String giinId, String compName, String compAddr,
        ...
        if (fiType == FiType.TRUST) {
            throw new IllegalStateException(
                    "信託部帳戶資料表尚無部門欄位可辨識歸屬，信託部 XML 產生邏輯尚未實作（待後續開發）");
        }

        List<FatcaXmlBuilder.AccountReportSpec> reports = queryAccountReports();
    ...
    private List<FatcaXmlBuilder.AccountReportSpec> queryAccountReports() {
        List<BaseRow> baseRows = jdbc.query(
                "SELECT DocRefID, AccountNumber, isOrg, AccountBalance, BalanceCurrCode " +
                "FROM XML_CorrectableAccountReport_Type ORDER BY AccountNumber",
        ...
        for (BaseRow base : baseRows) {
            ...
            if ((isOrg && org == null) || (!isOrg && person == null)) {
                log.warn("帳號 {} 找不到對應持有人資料（isOrg={}），略過此帳戶", base.accountNumber(), isOrg);
                continue;
            }
    ...
    /** 只取 AcctHolderType 有值的列（帳戶持有人本人），排除同一帳號下的實質受益人列。 */
    private Map<String, OrgHolder> findOrgHolders(List<String> accountNumbers) {
        ...
        List<Object[]> rows = jdbc.query(
                ...
                "WHERE AcctHolderType IS NOT NULL AND AccountNumber IN (" + placeholders + ")",
