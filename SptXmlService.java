...
public class SptXmlService {
    ...
    public String generateXml(String giinId, String compName, String compAddr,
        ...
        List<FatcaXmlBuilder.PoolReportSpec> poolReports = queryPoolReports(giinId);
        log.info("由 XML_* 舊表組出 {} 筆帳戶申報明細、{} 筆彙總（pool）申報明細（fiType={}）",
                reports.size(), poolReports.size(), fiType);
        ...
        if (reports.isEmpty() && poolReports.isEmpty()) {
            builder.nilReport();
        } else {
            reports.forEach(builder::addAccountReport);
            poolReports.forEach(builder::addPoolReport);
        }
    ...
    private record PoolReportRow(String docRefId, Integer accountCount, String poolReportType,
                                  BigDecimal poolBalance, String balanceCurrency) {}

    /** 查表組出彙總（pool）申報明細 —— 對應 IRS PoolReport（如美國指標之不合作帳戶、休眠帳戶），
     *  這些帳戶不逐戶列出明細，只申報彙總的帳戶數與帳戶餘額。 */
    private List<FatcaXmlBuilder.PoolReportSpec> queryPoolReports(String giin) {
        List<PoolReportRow> rows = jdbc.query(
                "SELECT DocRefID, AccountCount, AccountPoolReportType, PoolBalance, BalanceCurrCode " +
                "FROM XML_PoolReport WHERE giin = ? ORDER BY DocRefID",
                (rs, rowNum) -> new PoolReportRow(
                        rs.getString("DocRefID"),
                        rs.getObject("AccountCount", Integer.class),
                        rs.getString("AccountPoolReportType"),
                        rs.getBigDecimal("PoolBalance"),
                        rs.getString("BalanceCurrCode")
                ),
                giin
        );

        List<FatcaXmlBuilder.PoolReportSpec> result = new ArrayList<>();
        for (PoolReportRow row : rows) {
            if (row.accountCount() == null) {
                log.warn("彙總申報 DocRefID={} 缺少 AccountCount，略過此筆彙總申報", row.docRefId());
                continue;
            }
            // XML_PoolReport.AccountPoolReportType 是 CHAR(8)，Postgres 對定長不足的值會補右邊
            // 空白，查出來要 trim 掉才能跟 FatcaAcctPoolReportType_EnumType 的值比對。
            String rawType = row.poolReportType() != null ? row.poolReportType().trim() : null;
            PoolReportCode code = PoolReportCode.fromXmlCode(rawType).orElse(null);
            if (code == null) {
                log.warn("未知的 AccountPoolReportType={}（DocRefID={}，非 IRS 官方 FATCA201-206 代碼），" +
                        "略過此筆彙總申報", rawType, row.docRefId());
                continue;
            }
            result.add(FatcaXmlBuilder.PoolReportSpec.builder()
                    .docRefId(row.docRefId())
                    .accountCount(row.accountCount())
                    .poolReportType(code)
                    .poolBalance(Money.of(row.poolBalance(), row.balanceCurrency()))
                    .build());
        }
        return result;
    }
